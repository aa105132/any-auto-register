"""Kombai 2API 插件：WebSocket CHATv2 → OpenAI /v1/chat/completions 转换。

kombai 无原生 OpenAI 兼容端点，聊天走 WS 自定义协议（见 platforms/kombai/core.py ws_chat +
memory kombai_ws_protocol）。本插件把 OpenAI chat/completions 请求转成 WS CHATv2，
收 streamMessage 帧拼接文本，包装回 OpenAI 响应格式。

账号来源：account_manager.db（platform='kombai'，token=apiKeyToken/x-api-key）+
output/kombai_credentials.json + kombai_keys.txt。
"""
...
KOMBAI_OPENAI_BASE_URL = "https://api.assistant.app.kombai.com/v1"
KOMBAI_CHAT_COMPLETIONS_URL = f"{KOMBAI_OPENAI_BASE_URL}/chat/completions"
KOMBAI_MODELS_URL = f"{KOMBAI_OPENAI_BASE_URL}/models"

# 暴露给 OpenAI 客户端的模型 id（路由器 + 直选）
KOMBAI_PUBLIC_MODELS = list(KOMBAI_ROUTERS) + list(KOMBAI_MODELS)
KOMBAI_DEFAULT_MODEL = DEFAULT_MODEL

KOMBAI_IMPORT_SCHEMA = TwoAPIImportSchema(
    plugin="kombai",
    platform="kombai",
    token_fields=("api_key", "ai_api_token", "x_api_key", "token", "apikeytoken", "key"),
    email_fields=("email", "account", "username", "user_email"),
    user_id_fields=("user_id", "uid", "id", "org_id"),
    base_url_fields=("api_base", "ws_base", "base_url", "openai_compatible_api_base"),
    default_base_url=KOMBAI_OPENAI_BASE_URL,
    token_prefixes=(),
    min_token_length=32,
    credential_aliases=("api_key", "ai_api_token", "x_api_key", "token"),
    primary_token_field="api_key",
    metadata_defaults={"openai_compatible": False, "native_kombai": True, "transport": "websocket"},
)
...
class _KombaiStreamResponse:
    """WS 流式响应包装：iter_content 逐 chunk yield OpenAI SSE bytes，兼容 _iter_upstream_bytes。"""
...
def _looks_like_kombai_key(value: Any) -> bool:
    text = _safe_text(value)
    if len(text) < 32:
        return False
    if "@" in text or "://" in text or any(ch.isspace() for ch in text):
        return False
    return bool(all(ch.isalnum() or ch in "_-." for ch in text))


def _extract_api_key(record: dict[str, Any]) -> str:
    for key in ("api_key", "ai_api_token", "x_api_key", "token", "key", "apikeytoken", "primary_token"):
        value = _safe_text(record.get(key))
        if _looks_like_kombai_key(value):
            return value
    credentials = record.get("credentials")
    if isinstance(credentials, dict):
        for key in ("api_key", "ai_api_token", "x_api_key", "token", "key"):
...
def _credit_amount_from_record(record: dict[str, Any], *, default: float = 150.0) -> float:
    for key in ("credit_amount", "credits", "balance"):
        if key in record:
            return _safe_float(record.get(key), default)
    credit_info = record.get("credit_info") or record.get("subscription")
    if isinstance(credit_info, dict):
        for key in ("credits", "creditBalance", "remainingCredits", "amount", "balance"):
            if key in credit_info:
                return _safe_float(credit_info.get(key), default)
    return default


class KombaiTwoAPIPlugin:
    name = "kombai"
    display_name = "Kombai WS→OpenAI 代理"
    ...
    def log(self, message: str) -> None:
        ...
        with (log_dir / "kombai.log").open("a", encoding="utf-8") as handle:
            handle.write(line + "\n")

    def recent_logs(self, *, limit: int = 200) -> list[str]:
        file_path = self.data_dir / "twoapi_logs" / "kombai.log"
    ...
    def _credential_paths(self) -> list[Path]:
        return [
            self.data_dir / "kombai_credentials.json",
            self.data_dir / "kombai_account_result.json",
            self.data_dir / "kombai_e2e_result.json",
        ]
    ...
    def _load_accounts_from_files(self) -> list[TwoAPIAccount]:
        ...
        keys_path = self.data_dir / "kombai_keys.txt"
        if keys_path.exists():
            for index, raw in enumerate(keys_path.read_text(encoding="utf-8", errors="replace").splitlines(), start=1):
                ...
                key = next((part for part in parts if _looks_like_kombai_key(part)), "")
                if key:
                    email = next((part for part in parts if "@" in part), "") or f"kombai-key-{index}@local"
                    records.append({"email": email, "api_key": key, "source": "kombai_keys"})
    ...
    def _load_accounts_from_database(self) -> list[TwoAPIAccount]:
        ...
        try:
            ...
            self.log(f"读取 Kombai 账号数据库失败: {exc!r}")
            return []
        try:
            rows = connection.execute(
                "SELECT id, email, user_id FROM accounts WHERE lower(platform)='kombai' ORDER BY id ASC"
            ).fetchall()
            records: list[dict[str, Any]] = []
            for row in rows:
                ...
                credential_rows = connection.execute(
                    "SELECT key, value FROM account_credentials WHERE account_id=? AND provider_name='kombai'",
                    (account_id,),
                ).fetchall()
            ...
            self.log(f"读取 Kombai 数据库账号失败: {exc!r}")
            return []
        finally:
            connection.close()
    ...
    def _accounts_from_records(self, records: list[dict[str, Any]], *, source: str) -> list[TwoAPIAccount]:
        ...
        for index, record in enumerate(records, start=1):
            ...
            if not api_key:
                continue
            email = _safe_text(record.get("email")) or f"kombai-key-{index}@local"
            amount = _credit_amount_from_record(record, default=150.0)
            base_url = _safe_text(record.get("api_base") or record.get("base_url")) or KOMBAI_OPENAI_BASE_URL
            ...
            metadata = {
                ...
                "default_free_model": _safe_text(record.get("default_free_model")) or KOMBAI_DEFAULT_MODEL,
                "free_models": record.get("free_models") if isinstance(record.get("free_models"), list) else list(KOMBAI_PUBLIC_MODELS),
            ...
            accounts.append(
                TwoAPIAccount(
                    ...
                    base_url=base_url,
                    api_key=api_key,
                    handle=_safe_text(overview.get("plan") or metadata.get("org_id") or metadata.get("user_id")),
    ...
    def load_accounts(self) -> list[TwoAPIAccount]:
        ...
        self.log(f"加载 Kombai 账号 {len(self.accounts)} 个，其中 db={len(db_accounts)} file={len(file_accounts)}")
    ...
    def _account_is_eligible(self, account: TwoAPIAccount) -> bool:
        ...
        if not account.credit_ok or float(account.credit_amount or 0.0) < self.settings.min_credit:
            self.log(f"跳过不可用 Kombai 账号: {account.email} credit={account.credit_amount}")
            return False
        return True

    def select_account(self) -> TwoAPIAccount:
        if not self.settings.enabled:
            raise RuntimeError("Kombai 2API 已禁用")
        ...
        if not self.accounts and self.settings.auto_refill:
            ...
            self.load_accounts()
        if not self.accounts:
            raise RuntimeError("Kombai 账号池为空")
        ...
        for offset in range(total):
            ...
            self.log(f"选中 Kombai 账号: {account.email}")
            return account
        if self.settings.auto_refill:
            self.refill_accounts(count=1, concurrency=1)
            self.load_accounts()
        raise RuntimeError("没有可用 Kombai 账号：全部额度耗尽或凭据不可用")
        ...
        return LocalJSONResponse(
            {
                "object": "list",
                "data": [
                    {"id": model_id, "object": "model", "created": 0, "owned_by": "kombai"}
                    for model_id in KOMBAI_PUBLIC_MODELS
                ],
            }
        )
    ...
    def _build_openai_response(self, *, model: str, text: str, stop_reason: str, is_error: bool) -> dict[str, Any]:
        ...
        return {
            "id": f"chatcmpl-kombai-{uuid.uuid4().hex[:16]}",
            "object": "chat.completion",
            "created": int(time.time()),
            "model": model or KOMBAI_DEFAULT_MODEL,
            "choices": [
                {
                    "index": 0,
                    "message": {"role": "assistant", "content": text or ("[kombai error] " + stop_reason) if is_error else text},
    ...
    def _build_sse_chunks(self, *, model: str, text: str, stop_reason: str, is_error: bool) -> list[bytes]:
        chat_id = f"chatcmpl-kombai-{uuid.uuid4().hex[:16]}"
        created = int(time.time())
        finish = "stop"
        if stop_reason in ("max_tokens", "length"):
            finish = "length"
        ...
        chunks.append(self._sse_line({
            "id": chat_id, "object": "chat.completion.chunk", "created": created, "model": model or KOMBAI_DEFAULT_MODEL,
        ...
        if is_error:
            chunks.append(self._sse_line({"error": {"message": stop_reason, "type": "kombai_error"}}))
    ...
    def _run_ws_chat(self, account: TwoAPIAccount, payload: dict[str, Any]) -> dict[str, Any]:
        client = KombaiClient(proxy=None, log_fn=self.log)
        model = _safe_text(payload.get("model")) or KOMBAI_DEFAULT_MODEL
        ...
        result = client.ws_chat(
            account.api_key,
            messages=messages,
            model=model,
            stream=False,
            timeout=float(self.settings.request_timeout or 120.0),
            thread_id=thread_id,
            thinking_effort=str(payload.get("reasoning_effort") or payload.get("thinking_effort") or "medium"),
        )
        # 复用服务端分配的 thread_id 便于多轮
        if result.get("thread_id"):
            account.metadata = {**meta, "thread_id": result["thread_id"]}
    ...
    def forward_chat(self, payload: dict[str, Any], *, stream: bool = False) -> Any:
        ...
        for _ in range(attempts):
            ...
            try:
                result = self._run_ws_chat(account, payload)
                text = str(result.get("text") or "")
                stop_reason = str(result.get("stop_reason") or "end_turn")
                is_error = bool(result.get("is_error"))
                if is_error or (not text and stop_reason in ("ws_connect_failed", "error")):
                    account.last_status = f"chat_error:{stop_reason}"
                    self.log(f"Kombai WS chat 错误: {account.email} reason={stop_reason}")
                    last_error = RuntimeError(f"Kombai WS chat failed: {stop_reason}")
                    continue
                account.last_status = "chat_alive"
                if stream:
                    return _KombaiStreamResponse(
                        self._build_sse_chunks(model=result["model"], text=text, stop_reason=stop_reason, is_error=is_error)
                    )
                return LocalJSONResponse(
                    self._build_openai_response(model=result["model"], text=text, stop_reason=stop_reason, is_error=is_error)
                )
            except Exception as exc:
                last_error = exc
                account.last_error = repr(exc)
                account.last_status = "chat_failed"
                self.log(f"Kombai chat 转发失败: {account.email} {exc!r}")
                continue
        raise RuntimeError(str(last_error or "Kombai chat 转发失败"))

    @property
    def import_schema(self) -> TwoAPIImportSchema:
        return KOMBAI_IMPORT_SCHEMA
    ...
    def _append_imported_credentials_file(self, result: dict[str, Any]) -> None:
        ...
        for item in list(result.get("accounts") or []):
            ...
            rows.append(
                {
                    "email": _safe_text(item.get("email")) or "kombai-import@local",
                    "api_key": key,
                    "ai_api_token": key,
                    "x_api_key": key,
                    "source": _safe_text(result.get("source")) or "external_import",
                    "credit_amount": _safe_float(item.get("credit_amount"), 150.0),
                    "free_models": list(KOMBAI_PUBLIC_MODELS),
                    "ok": True,
                }
            )
        if not rows:
            return
        path = self.data_dir / "kombai_credentials.json"
    ...
    def import_accounts(
        ...
        *,
        ...
        self.accounts = []
        self.log(f"导入 Kombai 外部账号: created={result.get('created')} accepted={result.get('accepted')} skipped={result.get('skipped')}")
        return result
    ...
    def _push_target_import_url(self, target_url: str) -> str:
        ...
        if base.endswith("/2api/plugins/kombai/import") or base.endswith("/api/2api/plugins/kombai/import"):
            return base
        if base.endswith("/api"):
            return f"{base}/2api/plugins/kombai/import"
        return f"{base}/api/2api/plugins/kombai/import"
    ...
    def _account_to_push_record(self, account: TwoAPIAccount) -> dict[str, Any]:
        ...
        record: dict[str, Any] = {
            ...
            "x_api_key": account.api_key,
            "base_url": account.base_url or KOMBAI_OPENAI_BASE_URL,
            "api_base": account.base_url or KOMBAI_OPENAI_BASE_URL,
            "credit_amount": float(account.credit_amount or 0.0),
            "source": _safe_text(metadata.get("source")) or "kombai_local",
            "native_kombai": True,
            "openai_compatible": False,
            "transport": "websocket",
            "free_models": list(KOMBAI_PUBLIC_MODELS),
    ...
    def push_accounts(
        ...
        *,
        ...
        if not records:
            return {"ok": False, "pushed": 0, "target_url": import_url, "error": "没有匹配的 Kombai 账号可推送"}
        ...
        if not getattr(response, "ok", False):
            raise ValueError(f"推送 Kombai 账号失败: status={getattr(response, 'status_code', 0)} body={str(data)[:500]}")
        self.log(f"推送 Kombai 账号到远端完成: pushed={len(records)} target={import_url}")
    ...
    def refill_accounts(
        ...
        *,
        ...
        self.log(f"已创建 Kombai 自动补号任务: task_id={task.get('id')} count={resolved_count}")

"""Runbear 2API 插件：agent chat LLM → OpenAI /v1/chat/completions 转换。

runbear 无原生 OpenAI 兼容端点，2api 包装 app.runbear.io 的 agent chat LLM（主人指令：
"把里面 llm 包装成 OpenAI 兼容端点"）。注册后需创建 agent 关联模型(gpt5.5/5.4/Claude4.6)
→ assistant UUID → agent chat 端点。

端点待 workflow 抓包确认（见 platforms/runbear/core.py RunbearClient.chat TODO）。
本插件先搭骨架：账号加载 + forward_models + forward_chat（调 RunbearClient.chat）。
真实 chat 端点确认后补 core.py 的 create_agent/chat 实现。

账号来源：account_manager.db（platform='runbear'，token=access_token）+
output/runbear_credentials.json + runbear_keys.txt。
"""
...
RUNBEAR_OPENAI_BASE_URL = f"{APP_URL}/v1"
RUNBEAR_PUBLIC_MODELS = list(RUNBEAR_MODELS)
RUNBEAR_DEFAULT_MODEL = DEFAULT_MODEL

RUNBEAR_IMPORT_SCHEMA = TwoAPIImportSchema(
    plugin="runbear",
    platform="runbear",
    token_fields=("api_key", "ai_api_token", "access_token", "token", "key"),
    email_fields=("email", "account", "username", "user_email"),
    user_id_fields=("user_id", "uid", "id", "assistant_uuid"),
    base_url_fields=("api_base", "base_url", "openai_compatible_api_base"),
    default_base_url=RUNBEAR_OPENAI_BASE_URL,
    token_prefixes=(),
    min_token_length=20,
    credential_aliases=("api_key", "ai_api_token", "access_token", "token"),
    primary_token_field="access_token",
    metadata_defaults={"openai_compatible": False, "native_runbear": True},
)
...
class _RunbearStreamResponse:
    """SSE 流式响应包装：iter_content 逐 chunk yield OpenAI SSE bytes。"""
...
def _looks_like_runbear_token(value: Any) -> bool:
...
def _extract_token(record: dict[str, Any]) -> str:
    for key in ("access_token", "api_key", "ai_api_token", "token", "key", "primary_token"):
        value = _safe_text(record.get(key))
        if _looks_like_runbear_token(value):
            return value
    credentials = record.get("credentials")
    if isinstance(credentials, dict):
        for key in ("access_token", "api_key", "ai_api_token", "token", "key"):
...
class RunbearTwoAPIPlugin:
    name = "runbear"
    display_name = "Runbear agent→OpenAI 代理"
    ...
    def log(self, message: str) -> None:
        ...
        with (log_dir / "runbear.log").open("a", encoding="utf-8") as handle:
            handle.write(line + "\n")

    def recent_logs(self, *, limit: int = 200) -> list[str]:
        file_path = self.data_dir / "twoapi_logs" / "runbear.log"
    ...
    def _credential_paths(self) -> list[Path]:
        return [
            self.data_dir / "runbear_credentials.json",
            self.data_dir / "runbear_account_result.json",
            self.data_dir / "runbear_e2e_result.json",
        ]
    ...
    def _load_accounts_from_files(self) -> list[TwoAPIAccount]:
        ...
        keys_path = self.data_dir / "runbear_keys.txt"
        if keys_path.exists():
            for index, raw in enumerate(keys_path.read_text(encoding="utf-8", errors="replace").splitlines(), start=1):
                ...
                key = next((part for part in parts if _looks_like_runbear_token(part)), "")
                if key:
                    email = next((part for part in parts if "@" in part), "") or f"runbear-key-{index}@local"
                    records.append({"email": email, "access_token": key, "source": "runbear_keys"})
    ...
    def _load_accounts_from_database(self) -> list[TwoAPIAccount]:
        ...
        try:
            ...
            self.log(f"读取 Runbear 账号数据库失败: {exc!r}")
            return []
        try:
            rows = connection.execute(
                "SELECT id, email, user_id FROM accounts WHERE lower(platform)='runbear' ORDER BY id ASC"
            ).fetchall()
            records: list[dict[str, Any]] = []
            for row in rows:
                ...
                credential_rows = connection.execute(
                    "SELECT key, value FROM account_credentials WHERE account_id=? AND provider_name='runbear'",
                    (account_id,),
                ).fetchall()
            ...
            self.log(f"读取 Runbear 数据库账号失败: {exc!r}")
            return []
        finally:
            connection.close()
    ...
    def _accounts_from_records(self, records: list[dict[str, Any]], *, source: str) -> list[TwoAPIAccount]:
        ...
        for index, record in enumerate(records, start=1):
            ...
            if not token:
                continue
            email = _safe_text(record.get("email")) or f"runbear-key-{index}@local"
            amount = _safe_float(record.get("credit_amount"), 100.0)
            base_url = _safe_text(record.get("api_base") or record.get("base_url")) or RUNBEAR_OPENAI_BASE_URL
            ...
            metadata = {
                ...
                "assistant_uuid": _safe_text(record.get("assistant_uuid") or overview.get("assistant_uuid")),
                "default_free_model": _safe_text(record.get("default_free_model")) or RUNBEAR_DEFAULT_MODEL,
                "free_models": record.get("free_models") if isinstance(record.get("free_models"), list) else list(RUNBEAR_PUBLIC_MODELS),
            ...
            accounts.append(
                TwoAPIAccount(
                    ...
                    base_url=base_url,
                    api_key=token,
                    handle=_safe_text(metadata.get("assistant_uuid") or metadata.get("user_id")),
    ...
    def load_accounts(self) -> list[TwoAPIAccount]:
        ...
        self.log(f"加载 Runbear 账号 {len(self.accounts)} 个，其中 db={len(db_accounts)} file={len(file_accounts)}")
    ...
    def _account_is_eligible(self, account: TwoAPIAccount) -> bool:
        ...
        if not account.credit_ok or float(account.credit_amount or 0.0) < self.settings.min_credit:
            self.log(f"跳过不可用 Runbear 账号: {account.email} credit={account.credit_amount}")
            return False
        return True

    def select_account(self) -> TwoAPIAccount:
        if not self.settings.enabled:
            raise RuntimeError("Runbear 2API 已禁用")
        ...
        if not self.accounts and self.settings.auto_refill:
            ...
            self.load_accounts()
        if not self.accounts:
            raise RuntimeError("Runbear 账号池为空")
        ...
        for offset in range(total):
            ...
            self.log(f"选中 Runbear 账号: {account.email}")
            return account
        if self.settings.auto_refill:
            self.refill_accounts(count=1, concurrency=1)
            self.load_accounts()
        raise RuntimeError("没有可用 Runbear 账号：全部额度耗尽或凭据不可用")
        ...
        return LocalJSONResponse(
            {
                "object": "list",
                "data": [
                    {"id": model_id, "object": "model", "created": 0, "owned_by": "runbear"}
                    for model_id in RUNBEAR_PUBLIC_MODELS
                ],
            }
        )
    ...
    def _build_openai_response(self, *, model: str, text: str, stop_reason: str, is_error: bool) -> dict[str, Any]:
        ...
        if stop_reason in ("max_tokens", "length"):
            ...
            finish = "tool_calls"
        return {
            "id": f"chatcmpl-runbear-{uuid.uuid4().hex[:16]}",
            "object": "chat.completion",
            "created": int(time.time()),
            "model": model or RUNBEAR_DEFAULT_MODEL,
            "choices": [
                {
                    "index": 0,
                    "message": {"role": "assistant", "content": text or ("[runbear error] " + stop_reason) if is_error else text},
    ...
    def _build_sse_chunks(self, *, model: str, text: str, stop_reason: str, is_error: bool) -> list[bytes]:
        chat_id = f"chatcmpl-runbear-{uuid.uuid4().hex[:16]}"
        created = int(time.time())
        finish = "stop"
        if stop_reason in ("max_tokens", "length"):
            finish = "length"
        ...
        chunks.append(self._sse_line({
            "id": chat_id, "object": "chat.completion.chunk", "created": created, "model": model or RUNBEAR_DEFAULT_MODEL,
        ...
        if is_error:
            chunks.append(self._sse_line({"error": {"message": stop_reason, "type": "runbear_error"}}))
        chunks.append(b"data: [DONE]\n\n")
        return chunks

    def forward_chat(self, payload: dict[str, Any], *, stream: bool = False) -> Any:
        """转 OpenAI chat → runbear agent chat LLM。

        RunbearClient.chat 调 POST api.runbear.io/internal/http/assistants/<appId>/chat，
        收 Vercel AI SDK SSE（text-delta）聚合文本 → 包装 OpenAI chat.completion。
        流式则按字符切片模拟 SSE（非流式先返回完整文本）。
        """
        ...
        for _ in range(attempts):
            ...
            client = RunbearClient(proxy=None, log_fn=self.log)
            model = _safe_text(payload.get("model")) or RUNBEAR_DEFAULT_MODEL
            messages = payload.get("messages") or []
            if not isinstance(messages, list) or not messages:
                prompt = _safe_text(payload.get("prompt")) or "Hello"
                messages = [{"role": "user", "content": prompt}]
            meta = dict(account.metadata or {})
            assistant_uuid = _safe_text(meta.get("assistant_uuid"))
            try:
                result = client.chat(
                ...
                if is_error or (not text and stop_reason in ("error",)):
                    account.last_status = f"chat_error:{stop_reason}"
                    self.log(f"Runbear chat 错误: {account.email} reason={stop_reason} err={result.get('error','')[:80]}")
                    last_error = RuntimeError(f"Runbear chat failed: {stop_reason}")
                    continue
                account.last_status = "chat_alive"
                if stream:
                    return _RunbearStreamResponse(
                ...
                self.log(f"Runbear chat 转发失败: {account.email} {exc!r}")
                continue
        raise RuntimeError(str(last_error or "Runbear chat 转发失败"))

    @property
    def import_schema(self) -> TwoAPIImportSchema:
        return RUNBEAR_IMPORT_SCHEMA
    ...
    def _append_imported_credentials_file(self, result: dict[str, Any]) -> None:
        ...
        for item in list(result.get("accounts") or []):
            if not isinstance(item, dict):
                continue
            key = _safe_text(item.get("access_token") or item.get("api_key") or item.get("token"))
            if not key:
                continue
            rows.append(
                {
                    "email": _safe_text(item.get("email")) or "runbear-import@local",
                    "access_token": key,
                    "api_key": key,
                    "ai_api_token": key,
                    "source": _safe_text(result.get("source")) or "external_import",
                    "credit_amount": _safe_float(item.get("credit_amount"), 100.0),
                    "free_models": list(RUNBEAR_PUBLIC_MODELS),
                    "ok": True,
                }
            )
        if not rows:
            return
        path = self.data_dir / "runbear_credentials.json"
    ...
    def import_accounts(
        ...
        *,
        ...
        self.accounts = []
        self.log(f"导入 Runbear 外部账号: created={result.get('created')} accepted={result.get('accepted')} skipped={result.get('skipped')}")
        return result
    ...
    def _push_target_import_url(self, target_url: str) -> str:
        ...
        if base.endswith("/2api/plugins/runbear/import") or base.endswith("/api/2api/plugins/runbear/import"):
            return base
        if base.endswith("/api"):
            return f"{base}/2api/plugins/runbear/import"
        return f"{base}/api/2api/plugins/runbear/import"
    ...
    def _account_to_push_record(self, account: TwoAPIAccount) -> dict[str, Any]:
        ...
        record: dict[str, Any] = {
            ...
            "base_url": account.base_url or RUNBEAR_OPENAI_BASE_URL,
            "credit_amount": float(account.credit_amount or 0.0),
            "source": _safe_text(metadata.get("source")) or "runbear_local",
            "native_runbear": True,
            "openai_compatible": False,
            "free_models": list(RUNBEAR_PUBLIC_MODELS),
            "ok": bool(account.enabled and account.api_key),
        }
        for key in ("user_id", "assistant_uuid", "account_id"):
    ...
    def push_accounts(
        ...
        *,
        ...
        if not records:
            return {"ok": False, "pushed": 0, "target_url": import_url, "error": "没有匹配的 Runbear 账号可推送"}
        ...
        if not getattr(response, "ok", False):
            raise ValueError(f"推送 Runbear 账号失败: status={getattr(response, 'status_code', 0)} body={str(data)[:500]}")
        self.log(f"推送 Runbear 账号到远端完成: pushed={len(records)} target={import_url}")
    ...
    def refill_accounts(
        ...
        *,
        ...
        self.log(f"已创建 Runbear 自动补号任务: task_id={task.get('id')} count={resolved_count}")

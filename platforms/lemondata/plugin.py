...
from platforms.lemondata.core import DASHBOARD_URL, LLM_API_BASE, SITE_URL


@register
class LemonDataPlatform(BasePlatform):
    name = "lemondata"
    ...
    def _run_oauth(self, ctx) -> dict:
        from platforms.lemondata.browser_oauth import register_with_browser_oauth
    ...
    def _map_result(self, result: dict) -> RegistrationResult:
        ...
        balance_result = dict(result.get("balance_result") or {})
        balance_ok = bool(balance_result.get("ok")) and float(balance_result.get("amount") or 0.0) >= 1.0
        account_info = dict(result.get("account_info") or result.get("session") or {})
        ...
        return RegistrationResult(
            ...
            token=api_key,
            status=AccountStatus.REGISTERED if api_key and balance_ok else AccountStatus.INVALID,
            extra={
                "api_key": api_key,
    ...
    def build_protocol_mailbox_adapter(self):
        return ProtocolMailboxAdapter(
            ...
            worker_builder=lambda ctx, artifacts: __import__(
                "platforms.lemondata.protocol_mailbox",
                fromlist=["LemonDataProtocolMailboxWorker"],
            ).LemonDataProtocolMailboxWorker(
                proxy=ctx.proxy,
                log_fn=ctx.log,
    ...
    def get_platform_actions(self) -> list:
        return [
            ...
            {"id": "export_all_keys", "label": "导出全部 LemonData API Key", "params": []},
        ]
    ...
    def _write_key(api_key: str) -> Path:
        ...
        for path in (lemon_path, output_dir / "keys.txt", output_dir / "ai_api_tokens.txt"):
    ...
    def _export_all(cls) -> dict:
        ...
        with Session(engine) as session:
            for row in session.exec(select(AccountModel).where(AccountModel.platform == "lemondata")).all():

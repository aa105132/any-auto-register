...
class SwarmsBrowserMailboxWorker:
    ...
    def _trpc_get(self, session: requests.Session, path: str) -> Any:
        resp = session.get(f"https://swarms.world/api/trpc/{path}", timeout=30)
    ...
    def _trpc_post(self, session: requests.Session, path: str, payload: dict) -> Any:
        resp = session.post(f"https://swarms.world/api/trpc/{path}", json={"json": payload}, timeout=30)
        if resp.status_code >= 400:
            raise RuntimeError(f"{path} HTTP {resp.status_code}: {resp.text[:300]}")
        data = resp.json()
        if isinstance(data, dict) and data.get("error"):
        ...
        *,
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                page.goto(verify_link, wait_until="networkidle", timeout=60000)
                ...
                page.goto(ACCOUNT_URL, wait_until="networkidle", timeout=60000)
                page.wait_for_timeout(4000)
                page.goto(API_KEYS_URL, wait_until="networkidle", timeout=60000)
